...
The table below is a high-level overview:

## Requirements
------------

To participate in Hydro liquidity auctions, bidders should ideally support CosmWasm, but at a minimum have Interchain Accounts (ICA) enabled as a host and have an active IBC channel to Neutron. This ensures seamless integration with Hydro’s deployment infrastructure and enables secure, automated liquidity management. Projects without direct compatibility should establish the necessary connections before submitting a bid. ## Bid template
------------
...
- Designed to be compatible with CosmWasm, 
- Projects must have Interchain Accounts (ICA) enabled as a host, 
- Active IBC channel to Neuron is required for seamless integration.

**2. Connection to Surrounding Context**

- This chunk is closely related to the overall "Key Components and Processes" section in the document, specifically under the "Committee Oversight" and "Bid Submissions" sections.
- The chunk provides additional details on the requirements and setup prerequisites for participating in Hydro liquidity auctions.

**3. Requirements, Conditions, and Constraints**

- Bidder must support CosmWasm, with a minimum requirement of ICA-enabled host and active IBB channel to Neutron.
- Participation in Pilot Rounds requires approval from the Hydro committee and filling out the project bid template.
- Projects without direct compatibility must establish necessary connections before submitting a bid.
- Filling out the project bid template is mandatory for prospective bidders.
