...
--keyring-backend=test specifies the keyring backend to use Query contracts by code id To get a list of contracts instantiated from a specific Code ID, you can use the following command.

---

**LLM Contextual Output:**

This text chunk is explaining how `cw-storage-plus` builds upon the CosmWasm framework itself for storing and managing data in smart contracts.

**Exact technical details:**

* The **raw API** provided by CosmWasm's base storage system is used as a foundation.
* A **type-safe interface** simplifies interacting with stored data.
* Key management involves ensuring unique keys, prefixing individual values or collections, and handling namespace conflicts.
* Value serialization utilizes JSON through `serde-json-wasm`, requiring custom implementations for certain types.

**Context connection:**

This chunk builds upon the surrounding context by explaining how CosmWasm's storage system can be enhanced with additional features provided by `cw-storage-plus`. It discusses key considerations for optimal performance and data management in smart contracts, as well as the benefits of using a type-safe interface.

**Requirements, conditions, or constraints:**

* The text assumes familiarity with CosmWasm, storage systems, and specific requirements like unique keys and performance optimization.
* The use of `serde-json-wasm` for value serialization requires knowledge of JSON serialization and deserialization techniques.
* Custom implementations for certain data types (e.g., `serde::Serialize` and `serde::Deserialize`) are necessary.

The chunk is providing a detailed explanation of how to integrate `cw-storage-plus` with CosmWasm, highlighting key features and best practices for effective storage management in smart contracts.
